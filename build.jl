...
for filename in readdir("src/posts")
    ...
    output = replace(header, "{TITLE}" => title * " - Damien Gonot", "{DESCRIPTION}" => "$(first(excerpt, 297))...") * templated_string * footer
...
output = replace(header, "{TITLE}" => "Blog Posts - Damien Gonot", "{DESCRIPTION}" => "List of blog posts by Damien Gonot.") * content * footer
...
# Other routes
routes = [
    (destination = "index.html", source = "index.html", title = "Damien Gonot", description = "Homepage of Damien Gonot's personal website."),
    (destination = "about.html", source = "about.org", title = "About - Damien Gonot", description = "Learn more about Damien Gonot."),
]
...
for route in routes
    ...
    local output = replace(header, "{TITLE}" => title, "{DESCRIPTION}" => description) * content * footer
